...
app.innerHTML = `
  <div class="logo">
    <span class="logo-flag" title="Brasil">🇧🇷</span>
    <h1>Calculadora de Dias Úteis</h1>
  </div>
  <p>Calcule datas finais considerando dias úteis e feriados nacionais.</p>
  <form id="calculatorForm" autocomplete="off">
    ...
    <div class="form-group">
      ...
      <input type="number" id="businessDays" min="1" max="500" required placeholder="Ex: 10">
    </div>
    <button type="submit">Calcular</button>
  </form>
  <div id="result"></div>
  <div id="api-info">
    <span>Status da API: <span id="api-status">Verificando...</span></span>
  </div>
`;
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    // Show loading state
    resultDiv.innerHTML =
      '<span class="loading"></span> <span style="color:#888;">Calculando...</span>';
    ...
    try {
      ...
      if (data.success) {
        ...
        const brazilianStartDate = DateUtils.isoToBrazilian(
          data.data.startDate,
        );
        ...
        <div class="result-success">
          ...
          <hr>
          <div style="font-size: 13px; color: #6b7280; text-align:right;">* Apenas dias úteis e feriados nacionais</div>
        </div>
      `;
      } else {
        resultDiv.innerHTML = `
